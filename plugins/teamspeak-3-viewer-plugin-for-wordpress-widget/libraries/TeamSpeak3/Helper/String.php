 ...
 *
 * $Id: String.php 2010-01-10 22:52:12 sven $
 * 
 ...
 * 
 * @package  TeamSpeak3_Helper_String
...
class TeamSpeak3_Helper_String implements ArrayAccess, Iterator, Countable
{
  ...
  {
    return (strstr($this->string, $pattern) !== FALSE) ? TRUE : FALSE;
  }
  ...
  /**
   * Splits the string into substrings wherever $seperator occurs.
   *
   * @param  string  $seperator
   * @param  integer $limit
   * @return array
   */
  public function split($seperator, $limit = 0)
  {
    $parts = explode($seperator, $this->string, ($limit) ? intval($limit) : $this->count());
   ...
   * @return TeamSpeak3_Helper_String
   */
  public function section($seperator, $first = 0, $last = 0)
  {
    $sections = explode($seperator, $this->string);
    ...
    $string = implode($seperator, $sections);
  ...
  public function resize($size, $char = "\0")
  {
    ...
    elseif($chars > 0)
    {
      str_pad($this->string, $size, strval($char));
    }
