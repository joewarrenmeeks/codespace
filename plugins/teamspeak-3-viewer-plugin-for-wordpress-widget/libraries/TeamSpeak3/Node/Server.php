 ...
 *
 * $Id: Server.php 2010-01-10 22:52:12 sven $
 * 
...
class TeamSpeak3_Node_Server extends TeamSpeak3_Node_Abstract
{
  ...
  /**
   * Deletes the channel specified by ID.
   *
   * @param  integer $cid
   * @param  boolean $force
   * @return void
  ...
  /**
   * Returns detailed information about one or more specified files stored in a channels file repository.
   *
   * @param  integer $cid
   * @param  string  $cpw
   * @param  string  $path
  ...
  {
    return $this->execute("ftgetfileinfo", array("cid" => $cid, "cpw" => $cpw, "name" => $name))->toArray();
  }
  ...
  {
    $this->execute("ftrenamefile", array("cid" => $cid, "cpw" => $cpw, "name" => $name));
  }
  ...
  public function clientList()
  {
    ...
    {
      $clients = $this->request("clientlist -uid -away -voice -info -groups")->toAssocArray("clid");
   ...
   * @return TeamSpeak3_Helper_String
   */
  public function tokenCreate($type = TeamSpeak3::TOKEN_SERVERGROUP, $id1, $id2 = 0)
  {
    $token = $this->execute("tokenadd", array("tokentype" => $type, "tokenid1" => $id1, "tokenid2" => $id2))->toList();
